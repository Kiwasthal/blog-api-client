...
const SinglePost = ({ post, index, animate }) => {
  const placeHolder = post.content.slice(0, 40) + '...';
...
const AllPosts = ({ posts }) => {
  ...
  const allPostControls = useAnimation();
  const [titleRef, titleInView] = useInView();
  const [allPostsRef, allPostsInView] = useInView();
  ...
  useEffect(() => {
    if (allPostsInView) allPostControls.start('visible');
  }, [allPostControls, allPostsInView]);
